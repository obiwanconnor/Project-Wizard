...
namespace Cainos.CustomizablePixelCharacter
{
    public class PixelCharacterController : MonoBehaviour
    {
        ...
        [FoldoutGroup("Movement"), ShowIf("jumpEnabled")] public float airGravityMul = 2.0f;                        // extra multiplier applied to gravity for the whole time the character is airborne, on top of jumpGravityMul/fallGravityMul above; real-world gravity (Physics2D.gravity, ~9.81) reads as low-gravity/floaty for a human-scale 2D platformer, this compensates so jumps feel snappier without touching the project's global Physics2D gravity (which would also affect thrown weapons, buoyancy, etc.)
        [Space]
        ...
        private void JumpUpdate()
        {
            ...
            if ( IsInAir )
            {
                //these corrections are relative to the airGravityMul baseline gravityScale already
                //applied above (and by the engine), so effective multiplier while held-and-rising is
                //airGravityMul * jumpGravityMul, and airGravityMul * fallGravityMul otherwise
                if (inputJump && velocity.y > 0)
                {
                    velocity.y += Physics2D.gravity.y * gravityScale * (jumpGravityMul - 1.0f) * Time.fixedDeltaTime;
                }
                //released early while still rising, or actually falling: apply the heavier
                //fall gravity for the rest of the arc instead of just until velocity.y hits 0
                else
                {
                    velocity.y += Physics2D.gravity.y * gravityScale * (fallGravityMul - 1.0f) * Time.fixedDeltaTime;
                }
            }
        }
        ...
        private void GroundCheck()
        {
            ...
            gravityScale = isGrounded ? 0.0f : airGravityMul;
